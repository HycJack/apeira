...
The `Agent` interface currently extends `AgentChannel`, `AgentQueue`, and adds state/lifecycle methods (`getInput`, `setInput`, `init`, `stop`). A proposal to narrow the caller-facing seam to only `send(input) → ReadableStream<AgentEvent>` and `stop()` — absorbing `run()`, `channel`, and `queue` into the implementation — was rejected.
